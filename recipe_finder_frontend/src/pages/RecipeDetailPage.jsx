...
import { Box, Grid, Title, Text, Image, Group, Badge, Button, Tabs, List, ActionIcon, Divider, Paper, Avatar, Rating, Textarea } from '@mantine/core';
import { IconClock, IconUsers, IconChefHat, IconStar, IconHeart, IconBookmark, IconPrinter, IconShare, IconMapPin, IconArrowLeft } from '@tabler/icons-react';
      ...
      <Box p="xl">
        ...
        <Grid>
          ...
          <Grid.Col span={{ base: 12, md: 8 }}>
            <Box>
              <Title order={1} mb="xs">{recipeData.title}</Title>
              
              <Group mb="md">
                <Group spacing={8}>
                  <Avatar src={recipeData.authorImage} radius="xl" size="sm" />
                  <Text size="sm" fw={500}>By {recipeData.author}</Text>
                </Group>
                
                <Group spacing={5}>
                  <IconMapPin size={14} />
                  <Text size="sm" c="dimmed">{recipeData.location}</Text>
                </Group>
                
                <Group spacing={5}>
                  <IconStar size={14} color="#FFD700" />
                  <Text size="sm">{recipeData.rating}</Text>
                  <Text size="sm" c="dimmed">({recipeData.reviewCount} reviews)</Text>
                </Group>
              </Group>
              ...
              <Box mb="xl">
                <Title order={3} mb="md">Reviews ({recipeData.reviews.length})</Title>
            ...
            <Paper p="md" withBorder>
              ...
              {recipeData.relatedRecipes.map((recipe) => (
                <Group key={recipe.id} mb="md" noWrap>
                  ...
                  />
                  <Box>
                    <Text fw={500} component={Link} to={`/recipe/${recipe.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      {recipe.title}
                    </Text>
